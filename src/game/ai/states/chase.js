...
var ChaseEnemy = State.extend({
    ...
    execute: function(npc, dTime) {
        ...
        if (npc.health <= 0 || npc.template.disabled) {
            return;
        }
        ...
        if ((((this.enemy.id > 0 && (this.enemy.socket.disconnected || this.enemy.chInvisibleByMonsters)) || this.enemy.health <= 0 || this.chaseTimeBeforeGivingUp <= 0 || (npc.template.spawnguardradius > 0 && VectorDistance(npc.startPosition, npc.position) > npc.template.spawnguardradius)) && this.minimumChaseTime <= 0)) {
            ...
            if (this.chaseTimeBeforeGivingUp > 0) {
                this.chaseTimeBeforeGivingUp -= dTime;
            }
